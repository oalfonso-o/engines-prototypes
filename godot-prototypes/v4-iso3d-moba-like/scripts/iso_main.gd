...
@onready var center_spot_light: SpotLight3D = $CenterSpotLight
...
func _ready() -> void:
	...
	player.attach_world(projectiles_root, targets_root, iso_map)
...
func _setup_center_spotlight() -> void:
	var focus: Vector3 = iso_map.map_center
	var max_span: float = max(iso_map.map_width * iso_map.tile_size, iso_map.map_height * iso_map.tile_size)
	center_spot_light.global_position = focus + Vector3(0.0, max(18.0, iso_map.level_height * 7.5), 0.0)
	center_spot_light.rotation_degrees = Vector3(-90.0, 0.0, 0.0)
	center_spot_light.light_color = Color("dff6ff")
	center_spot_light.light_energy = 1.5
	center_spot_light.spot_range = max(28.0, max_span * 0.95)
	center_spot_light.spot_angle = 52.0
	center_spot_light.spot_angle_attenuation = 0.45
	center_spot_light.shadow_enabled = true
	center_spot_light.shadow_bias = 0.05
	center_spot_light.shadow_normal_bias = 1.2
...
func _setup_camera() -> void:
	...
	iso_camera.size = max(16.0, max_span * 0.48)
	iso_camera.near = 0.05
	iso_camera.far = 250.0
	_camera_offset = Vector3(22.0, 26.0, 22.0)
