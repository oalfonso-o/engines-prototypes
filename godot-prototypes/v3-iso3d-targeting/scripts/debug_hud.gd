...
func _ready() -> void:
	...
	info_label.size = Vector2(920.0, 320.0)
...
func _process(_delta: float) -> void:
	...
	var targeting_state: Dictionary = {}
	...
	if tracked_player != null:
		...
		targeting_state = tracked_player.get_targeting_debug_state()
	...
	var damage_per_shot := float(weapon_state.get("damage_per_shot", 0.0))
	...
	var is_targeting := bool(targeting_state.get("is_targeting", false))
	var hovered_target_name := str(targeting_state.get("hovered_target_name", ""))
	var target_point: Vector3 = targeting_state.get("target_point", Vector3.ZERO)
	var shot_mode := str(shot_state.get("mode", ""))
	var shot_status := str(shot_state.get("status", "idle"))
	var impact_position: Vector3 = shot_state.get("impact_position", Vector3.ZERO)
	...
	info_label.text = "\n".join([
		...
		"Weapon: %s ammo=%d/%d dmg=%.1f reload=%s %.2fs" % [weapon_name, ammo_in_magazine, reserve_ammo, damage_per_shot, str(reloading), reload_remaining],
		"Targeting: %s target=%s point=(%.2f, %.2f, %.2f)" % [str(is_targeting), hovered_target_name, target_point.x, target_point.y, target_point.z],
		"Last Shot: mode=%s status=%s dmg=%.1f impact=(%.2f, %.2f, %.2f)" % [shot_mode, shot_status, total_damage, impact_position.x, impact_position.y, impact_position.z],
	...
	crosshair_label.add_theme_color_override("font_color", Color("ffe58b") if is_targeting else Color("7ffbff"))
