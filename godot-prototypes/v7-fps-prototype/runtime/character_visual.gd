...
func _apply_pose(facing_direction: Vector3, delta: float) -> void:
	...
	var torso_roll: float = -local_reaction.x * _config.torso_tilt_strength * strength
	var torso_pitch: float = local_reaction.z * _config.torso_tilt_strength * strength
	var hand_lift: float = _config.hand_lift_strength * strength
	var hand_back: float = _config.hand_back_strength * strength
	var hand_outward: float = _config.hand_outward_strength * strength
	var foot_outward: float = _config.foot_outward_strength * strength
	var foot_tuck: float = _config.foot_tuck_strength * strength

	_parts.torso_attachment.rotation = Vector3(torso_pitch, 0.0, torso_roll)
	_parts.left_hand_attachment.position = _config.hand_offset(-1.0) + Vector3(-hand_outward, hand_lift, hand_back)
	_parts.right_hand_attachment.position = _config.hand_offset(1.0) + Vector3(hand_outward, hand_lift, hand_back)
	_parts.left_foot_attachment.position = _config.foot_offset(-1.0) + Vector3(-foot_outward, 0.0, foot_tuck)
	_parts.right_foot_attachment.position = _config.foot_offset(1.0) + Vector3(foot_outward, 0.0, foot_tuck)

	_reaction.head_lag_euler = _reaction.head_lag_euler.move_toward(
		Vector3(-torso_pitch * _config.head_follow_ratio, 0.0, -torso_roll * _config.head_follow_ratio),
...
class ReactionState:
	...
	func local_direction(global_basis: Basis, facing_direction: Vector3) -> Vector3:
		var local_direction: Vector3 = global_basis.inverse() * _world_direction
