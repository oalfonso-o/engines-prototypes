...
const OK_MESSAGE: String = "v6-character-prototype tests OK"
const TEST_CLEANUP_GROUP: String = "v6_test_cleanup"
...
class TestContext:
	...
	func instantiate_main_scene() -> Node3D:
		...
		var main_scene: Node3D = instance as Node3D
		if main_scene == null:
			push_error("res://main.tscn root is not a Node3D")
			if instance != null:
				instance.queue_free()
			return null
