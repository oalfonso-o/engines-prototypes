...
const RUNTIME_PATH: String = "res://data/runtime_world.json"
...
func _build_mesh(data: Dictionary) -> ArrayMesh:
	...
	for material_key_variant: Variant in materials:
		...
		for quad_variant: Variant in quad_list:
			...
			_append_runtime_quad(vertices, normals, uvs, indices, quad)
		var arrays: Array = []
...
func _append_runtime_quad(vertices: PackedVector3Array, normals: PackedVector3Array, uvs: PackedVector2Array, indices: PackedInt32Array, quad: Array) -> void:
	var origin: Vector3 = _grid_point(Vector3i(int(quad[0]), int(quad[1]), int(quad[2])))
...
func _build_collision(world_body: StaticBody3D, boxes: Array) -> void:
	for box_variant: Variant in boxes:
		...
		var y: int = int(box[1])
		var z: int = int(box[2])
		var width: int = int(box[3])
		var height: int = int(box[4])
		var depth: int = int(box[5])
...
func _grid_point(point: Vector3i) -> Vector3:
	return Vector3(_grid_x(point.x), _grid_y(point.y), _grid_z(point.z))
