...
typedef enum {
    BLOCK_KIND_NONE = 0,
    BLOCK_KIND_AUTO_STEP_ELIGIBLE = 1,
    BLOCK_KIND_ALWAYS_BLOCKING = 2,
} BlockKind;
...
static GroundCandidate select_ground_candidate(float sample_x, float sample_z, float reference_feet_y, float max_below, float max_above, CollisionScene collision_scene) {
    ...
    for (int i = 0; i < collision_scene.box_count; i++) {
        ...
        if (floor_y < (reference_feet_y - max_below - collision_scene.floor_epsilon)) {
            continue;
        }
        if (floor_y > (reference_feet_y + max_above + collision_scene.floor_epsilon)) {
            continue;
        }
...
static bool spawn_candidate_is_valid(Vec3 candidate, const PlayerControllerConfig* config, CollisionScene collision_scene, float ground_search_distance, Vec3* out_grounded_spawn) {
    GroundCandidate ground = select_ground_candidate(candidate.x, candidate.z, candidate.y, ground_search_distance, 0.0f, collision_scene);
    if (!ground.valid) {
        return false;
    }
...
static float resolve_horizontal_axis(PlayerState* state, const PlayerControllerConfig* config, CollisionScene collision_scene, char axis, float delta_axis) {
    ...
    const GroundCandidate auto_step_ground = select_ground_candidate(
        candidate_x,
        candidate_z,
        current_feet_y,
        collision_scene.ground_snap_distance,
        collision_scene.max_auto_step_height,
        collision_scene
    );
    const bool auto_step_to_higher_ground = auto_step_ground.valid &&
        auto_step_ground.floor_y > (current_feet_y + collision_scene.floor_epsilon);
    ...
    for (int i = 0; i < collision_scene.wedge_count; i++) {
        ...
        BlockKind block_kind = BLOCK_KIND_NONE;
        ...
        if (axis == 'x') {
            ...
            if (delta_axis > 0.0f) {
                if (wedge->type_id == collision_scene.wedge_type_id_pz || wedge->type_id == collision_scene.wedge_type_id_nz) {
                    block_kind = BLOCK_KIND_ALWAYS_BLOCKING;
                    block_coordinate = wedge->min_x - config->radius;
                } else if (wedge->type_id == collision_scene.wedge_type_id_nx) {
                    block_kind = BLOCK_KIND_AUTO_STEP_ELIGIBLE;
                    block_coordinate = wedge->min_x - config->radius;
                }
            } else {
                if (wedge->type_id == collision_scene.wedge_type_id_pz || wedge->type_id == collision_scene.wedge_type_id_nz) {
                ...
                } else if (wedge->type_id == collision_scene.wedge_type_id_px) {
                    block_kind = BLOCK_KIND_AUTO_STEP_ELIGIBLE;
                    block_coordinate = wedge->max_x + config->radius;
                }
            }
            if (block_kind == BLOCK_KIND_NONE || !interval_overlaps(candidate_x - config->radius, candidate_x + config->radius, wedge->min_x, wedge->max_x, collision_scene.horizontal_epsilon)) {
                continue;
            }
        } else {
            ...
            if (delta_axis > 0.0f) {
                if (wedge->type_id == collision_scene.wedge_type_id_px || wedge->type_id == collision_scene.wedge_type_id_nx) {
                    block_kind = BLOCK_KIND_ALWAYS_BLOCKING;
                    block_coordinate = wedge->min_z - config->radius;
                } else if (wedge->type_id == collision_scene.wedge_type_id_nz) {
                    block_kind = BLOCK_KIND_AUTO_STEP_ELIGIBLE;
                    block_coordinate = wedge->min_z - config->radius;
                }
            } else {
                ...
                } else if (wedge->type_id == collision_scene.wedge_type_id_pz) {
                    block_kind = BLOCK_KIND_AUTO_STEP_ELIGIBLE;
                    block_coordinate = wedge->max_z + config->radius;
                }
            }
            if (block_kind == BLOCK_KIND_NONE || !interval_overlaps(candidate_z - config->radius, candidate_z + config->radius, wedge->min_z, wedge->max_z, collision_scene.horizontal_epsilon)) {
                continue;
            }
        }

        if (block_kind == BLOCK_KIND_AUTO_STEP_ELIGIBLE && auto_step_to_higher_ground) {
            continue;
        }
...
void player_update(PlayerState* state, PlayerInputState* input, const PlayerControllerConfig* config, CollisionScene collision_scene, float delta_time) {
    ...
    const float ground_reference_feet_y = state->vertical_velocity <= 0.0f ? pre_vertical_feet_y : state->position.y;
    ...
    if (ground.valid) {
        if (state->position.y <= (ground.floor_y + collision_scene.vertical_epsilon) ||
            ...
            ((ground.floor_y - state->position.y) <= (collision_scene.max_auto_step_height + collision_scene.floor_epsilon))) {
