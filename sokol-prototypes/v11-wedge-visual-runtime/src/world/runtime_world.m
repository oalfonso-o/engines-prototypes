...
static void append_runtime_triangle(RuntimeWorld* world, int* vertex_cursor, int* index_cursor, const char* material_key, VoxelTriangle triangle) {
    ...
    const MaterialColor base_color = material_color_for_key(material_key);
    const float shade = face_shade_for_normal(triangle.normal_x, triangle.normal_y, triangle.normal_z);
    const float r = base_color.r * shade;
    const float g = base_color.g * shade;
    const float b = base_color.b * shade;
    const uint32_t base_index = (uint32_t)(*vertex_cursor);
...
bool runtime_world_load(const char* artifact_path, RuntimeWorld* out_world) {
    ...
    @autoreleasepool {
        ...
        if (![materials isKindOfClass:[NSArray class]] || ![quads_by_material isKindOfClass:[NSDictionary class]] || ![triangles_by_material isKindOfClass:[NSDictionary class]] || ![boxes isKindOfClass:[NSArray class]]) {
            return false;
        }
        ...
        for (NSString* material in materials) {
            ...
            if ([material_triangles isKindOfClass:[NSArray class]]) {
                for (NSArray* source_triangle in material_triangles) {
                    ...
                    append_runtime_triangle(out_world, &vertex_cursor, &index_cursor, material_key, triangle);
                }
            }
        }
