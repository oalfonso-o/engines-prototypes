...
static const char* RUNTIME_PATH = "data/runtime_mesh.txt";
static const float CAMERA_MOUSE_SENSITIVITY = 0.0035f;
static const float CAMERA_MOVE_SPEED = 2.2f;
static const float CAMERA_EYE_HEIGHT = 0.0f;
...
static bool load_runtime_mesh(const char* path, runtime_mesh_t* out_mesh) {
    ...
    char token[64] = {0};
    if (fscanf(file, "%63s", token) != 1 || strcmp(token, "RUNTIME_CUBE_V1") != 0) {
        fclose(file);
        return false;
    }
    if (fscanf(file, "%63s %d", token, &out_mesh->cube_count) != 2 || strcmp(token, "cube_count") != 0) {
        fclose(file);
        return false;
    }
...
static void init(void) {
    ...
    state.camera_position = vec3_make(0.0f, CAMERA_EYE_HEIGHT, 3.0f);
...
sapp_desc sokol_main(int argc, char* argv[]) {
    ...
    return (sapp_desc){
        ...
        .sample_count = 4,
        .high_dpi = true,
        .window_title = "sokol v4 single cube runtime",
        .logger.func = slog_func,
    };
}
