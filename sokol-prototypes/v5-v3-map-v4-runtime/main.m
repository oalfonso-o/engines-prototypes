...
static bool load_runtime_mesh(const char* path, runtime_mesh_t* out_mesh) {
    ...
    char token[64] = {0};
    if (fscanf(file, "%63s", token) != 1 || strcmp(token, "RUNTIME_VOXEL_MESH_V1") != 0) {
        fclose(file);
        return false;
    }
...
static void init(void) {
    ...
    state.camera_position = vec3_make(0.0f, 0.6f, 2.2f);
    state.camera_yaw = 0.0f;
    state.camera_pitch = -0.15f;
}
...
sapp_desc sokol_main(int argc, char* argv[]) {
    ...
    return (sapp_desc){
        ...
        .sample_count = 4,
        .high_dpi = true,
        .window_title = "sokol v5 v3-map v4-runtime",
        .logger.func = slog_func,
    };
}
