...
static Settings launch_settings;
static bool launch_settings_loaded = false;

static const char* settings_path_from_environment(void) {
    const char* settings_path = getenv("SETTINGS_PATH");
...
static void ensure_launch_settings_loaded(void) {
    if (launch_settings_loaded) {
        return;
    }
    const char* settings_path = settings_path_from_environment();
    if (!settings_load(settings_path, &launch_settings)) {
...
static void app_init(void) {
    ...
    runtime_world_append_ground_plane(
        &game_app.world,
        game_app.settings.world.ground_half_size,
        game_app.settings.render.ground_plane_color
    );
    world_renderer_init(&renderer, &game_app.world, game_app.settings.render.clear_color);
    player_state_init(&game_app.player, game_app.world.spawn, game_app.settings.camera.initial_pitch);
}
...
static void app_frame(void) {
    ...
    const Mat4 projection = mat4_perspective(
        ...
        game_app.settings.camera.near_plane,
        game_app.settings.camera.far_plane
    );
...
static void app_cleanup(void) {
    ...
    settings_release(&game_app.settings);
    sg_shutdown();
}
