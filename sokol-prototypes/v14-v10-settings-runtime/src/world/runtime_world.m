...
static void append_runtime_quad(RuntimeWorld* world, int* vertex_cursor, int* index_cursor, const Settings* settings, const char* material_key, VoxelQuad quad) {
    ...
    Color4 base_color = {0};
    if (!settings_material_color(settings, material_key, &base_color)) {
        abort();
    }
    const float shade = settings_face_shade_for_normal(settings, quad.normal_x, quad.normal_y, quad.normal_z);
...
bool runtime_world_load(const char* artifact_path, const Settings* settings, RuntimeWorld* out_world) {
...
void runtime_world_append_ground_plane(RuntimeWorld* world, float half_size, Color4 color) {
    ...
    const Vertex ground_vertices[4] = {
        { -half_size, 0.0f, -half_size, color.r, color.g, color.b, color.a },
        { -half_size, 0.0f,  half_size, color.r, color.g, color.b, color.a },
        {  half_size, 0.0f,  half_size, color.r, color.g, color.b, color.a },
        {  half_size, 0.0f, -half_size, color.r, color.g, color.b, color.a },
    };
